...
import { getMessages, sendMessage, getConversations, acceptConversation, initiateSale, confirmSale, rateSellerForConversation } from '../api/messages.jsx';
...
const Messages = () => {
  ...
  const handleInitiateSale = async () => {
    ...
    try {
      ...
      toast.success('Sale request sent to buyer! They will receive a notification.');
      
      // Refresh conversations and messages
      const convos = await getConversations(token);
      setConversations(convos);
      const updatedConvo = convos.find(c => c._id === selectedConversation._id);
      if (updatedConvo) {
        ...
        if (otherParticipant) {
          const msgs = await getMessages(otherParticipant._id, updatedConvo.listing?._id, token);
          setMessages(msgs);
        }
      }
    } catch (err) {
  ...
  const isBuyerInSelected = selectedConversation && String(selectedConversation?.listing?.seller?._id || selectedConversation?.listing?.seller) !== String(user?.id || user?._id);
  const shouldForceRating = !!(selectedConversation && selectedConversation.saleStatus === 'confirmed' && isBuyerInSelected && selectedConversation.buyerRated === false);
                      ...
                      <div className="flex-1">
                        ...
                        {convo.createdAt && (
                          <p className="text-xs text-gray-500 mt-1">
                            Requested: {new Date(convo.createdAt).toLocaleString([], { 
                              month: 'short', 
                              day: 'numeric', 
                              hour: '2-digit', 
                              minute: '2-digit' 
                ...
                {selectedConversation.status === 'accepted' && 
                 ...
                 !selectedConversation.listing.isSold && (
                  <>
                    ...
                    {String(selectedConversation.listing.seller?._id || selectedConversation.listing.seller) === String(user?.id || user?._id) && 
                     selectedConversation.saleStatus !== 'pending_confirmation' && 
                    ...
                    {String(selectedConversation.listing.seller?._id || selectedConversation.listing.seller) !== String(user?.id || user?._id) && 
                     selectedConversation.saleStatus === 'pending_confirmation' && (
                      ...
                      >
                        <FaCheckCircle />
                        {confirmingSale ? 'Confirming...' : 'Confirm Purchase'}
                      </button>
                    )}
                      ...
                      {selectedConversation.saleStatus === 'pending_confirmation' && 
                       String(selectedConversation.listing.seller?._id || selectedConversation.listing.seller) !== String(user?.id || user?._id) && (
                        <p className="text-xs text-yellow-700 mt-1 font-medium">
                          ⚠️ Sale confirmation requested - Please confirm your purchase
                        </p>
                      )}
                    </div>
                    {selectedConversation.listing.isSold || selectedConversation.saleStatus === 'confirmed' ? (
      ...
      {shouldForceRating && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">Please rate your experience with the seller. This helps the community.</p>
